...
const CartMain = ({ cart }) => {
  ...
  const handleIncrement = async (itemId) => {
    ...
    try {
      ...
      console.log(`http://localhost:3000/api/carts/${userId}/items/${itemId}`)
  
      const response = await axios.post(`http://localhost:3000/api/carts/${userId}/items/${itemId}`, {
        quantity: 1,
      }, {
        headers: {
      ...
      >
        <div className="w-full">
          <div className="360:text-center 360:py-4 360:border-b border-gray-500 lg:mb-6 lg:text-left lg:py-0 lg:border-0 xl:mb-10">
            <h1 className="font-medium 360:text-xl lg:text-2xl xl:text-[3.5vh]">Bag</h1>
          </div>
              ...
              {cartItems.map((item) => (
                ...
                >
                  <div className="item-content flex">
                    ...
                    <div className="ml-3">
                      <h2 className="font-medium xl:text-[3vh]">{item.itemId.name}</h2>
                      <p className="font-medium text-sm text-gray-500 xl:text-[2vh] xl:mt-1">
                      {item.itemId.gender === 'Male' ? "Men's" : item.itemId.gender === 'Female' ? "Women's" : ""}
                      </p>
                      <p className="mt-1 text-sm text-gray-500 xl:text-[2vh]">{item.itemId.sportsAndActivities}</p>
                    </div>
                    <div className="font-medium ml-auto xl:text-[3vh]">
                      <h2>${item.itemId.price}</h2>
                    </div>
                  </div>

                  <div className="action-button inline-flex gap-5 border border-gray-400 p-2 rounded-3xl mt-4 lg:p-[2vh] xl:rounded-full lg:gap-[4vn]">
                    <div className="action-icon delete-button">
                      <button onClick={() => deleteCartItem(item.itemId._id)}>
                        <svg
                          ...
                          width="24px"
                          fill="#000000"
                          className="lg:w-[3.5vh] lg:h-[3.5vh]"
                        >
                    ...
                    <div className="action-icon active-count">
                      <span className="xl:text-[2.5vh]">{item.quantity}</span>
                    </div>
        ...
        <div className="price-summary text-sm font-medium lg:min-w-[20vw]">
          <div className="summary-desc border-b 360:border-t lg:border-t-0 border-gray-300">
            <div className="360:py-4 lg:mb-6 lg:text-left lg:py-0 lg:border-0 xl:mb-10">
              <h1 className="font-medium 360:text-xl lg:text-2xl xl:text-[3.5vh]">Summary</h1>
            </div>
            ...
            <div className="promo-code xl:pb-2">
              ...
              >
                <p className="xl:text-[2.5vh]">Do you have a Promo Code?</p>
                <svg
            ...
            <div className="subtotal py-2 xl:py-4 flex justify-between">
              <h2 className="xl:text-[2.5vh]">Subtotal</h2>
              <p className="xl:text-[2.5vh]"> {`$${cart.totalPrice}`} </p>
            </div>

            <div className="shipping-and-handling py-2 xl:py-4 flex justify-between">
              <h2 className="xl:text-[2.5vh]">Estimated Shipping & Handling</h2>
              <p className="xl:text-[2.5vh]">Free</p>
            </div>

            <div className="estimated-tax pb-4 pt-2 xl:pb-8 xl:pt-4 flex justify-between">
              <h2 className="xl:text-[2.5vh]">Estimated Tax</h2>
              <p className="xl:text-[2.5vh]">-</p>
            </div>
          </div>

          <div className="total py-4 lg:border-b border-gray-300 flex justify-between xl:py-8">
            <h2 className="xl:text-[2.5vh]">Total</h2>
            <p className="xl:text-[2.5vh]"> {`$${cart.totalPrice}`} </p>
          </div>
          ...
          <div className="checkout mt-6 xl:mt-10">
            <button
              onClick={orderClick}
              className="w-full text-center 360:text-lg py-4 lg:text-sm xl:text-[3vh] xl:py-[3vh] bg-black text-white rounded-3xl"
            >
              Order Now
            </button>
          </div>
